open Graphics ;;

type size = int * int ;;
(* images are lists of lists of floats between 0. (white) and 1. (black) *)
type image = float list list ;;

(* show the image *)
let depict (img: image) : unit =
  open_graph "";
  clear_graph ();
  let x, y = List.length (List.hd img), List.length img in resize_window x y;
  let depict_pix v r c =
    let lvl = int_of_float (255. *. (1. -. v)) in set_color (rgb lvl lvl lvl);
    plot c (y - r) in
    List.iteri (fun r row -> List.iteri (fun c pix -> depict_pix pix r c) row) img;
    Unix.sleep 2;
    close_graph () ;;

(* threshold thershold image -- image where pixels above the threshold
value are black *)
let threshold (img : image) (threshold : float) : image =
  List.map (fun row -> List.map (fun v -> if v <= threshold then 0. else 1.)
                row) img ;;

(* dither max image -- dithered image *)
let dither (img : image) : image =
  List.map
    (fun row ->
     List.map
       (fun v -> if v > Random.float 1.
                 then 1.
                 else 0.) row) img ;;

(* error diffusion *)
let error_diffusion (img : image) (threshold : float) : image =
  let nimg = Array.of_list (List.map Array.of_list img) in
  let rows = Array.length nimg in
  let col = Array.length (Array.get nimg 0) in
  let copy = Array.copy nimg in
  for i = 0 to rows do
    for j = 0 to col do
      if copy.(i).(j) <= threshold then copy.(i).(j) <- 0. else copy.(i).(j) <- 1.
    done
  done;
  Array.to_list (Array.map Array.to_list copy)
  ;;

let mona = Monalisa.image ;;
depict mona ;;

let mona_threshold = threshold mona 0.75 ;;
depict mona_threshold ;;

let mona_dither = dither mona ;;
depict mona_dither ;;
